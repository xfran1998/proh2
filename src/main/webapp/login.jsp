<!DOCTYPE html>
<html lang="es" dir="ltr">
	<head>
		<meta
			name="viewport"
			content="width=device-width, user-scalable=no, initial-scale=1.0"
		/>
		<meta charset="utf-8" />
		<link rel="stylesheet" type="text/css" href="css/login.css" />
		<link
			href="https://fonts.googleapis.com/css2?family=Montserrat:wght@400;700;800&display=swap"
			rel="stylesheet"
		/>
		<%
		String type = (String) request.getAttribute("type");
		String msg = (String) request.getAttribute("msg");

		if (type != null && type.equals("info") && msg != null){
		%>
		<script type="text/javascript" defer>
		   var s="<%=msg%>"; 
		   setTimeout( () => {
			   alert(s); 		
			   console.log("log__", s);		   
		   }, 1000);
		</script>
		<%
		}
		%>
	</head>
	<body>
		<div class="main no-color-trans">
			<div class="container b-container no-color-trans" id="b-container">
				<form	class="form" id="a-form" method="POST" action="registro.html"	autocomplete="off">
					<h2 class="form_title title">Crea tu cuenta</h2>
					<div class="form__icons">
						<img
							class="form__icon"
							src="data:image/svg+xml;base64,"
							alt="form_icon"
						/><img
							class="form__icon"
							src="data:image/svg+xml;base64,"
							alt="form_icon"
						/><img
							class="form__icon"
							src="data:image/svg+xml;base64,"
							alt="form_icon"
						/>
					</div>
					<%
						// Conseguimos el msg de error desde el request
						if (type != null && type.equals("register") && msg != null) { //Eliminamos el mensaje consumido
						%>
						<p><span class="form-info"> <%=msg%> </span></p>
						<%
						}
					%>
					<span class="form__span">o registra tu correo</span
					><input
						class="form__input"
						type="text"
						id="a-user"
						name="user"
						placeholder="Usuario"
					/><input
						class="form__input"
						type="email"
						id="a-email"
						name="email"
						placeholder="Email"
					/><input
						class="form__input"
						type="password"
						id="a-password"
						name="password"
						placeholder="Password"
					/><input
					class="form__input"
					type="password"
					id="a-confirm-password"
					name="confirm-password"
					placeholder="Confirm Password"
				/><input type="submit" value="Registro" name="submit" class="form__button button submit">
				</form>
			</div>
			<div class="container a-container" id="a-container">
				<form class="form" id="b-form" method="POST" action="login.html">
					<h2 class="form_title title">Usuario</h2>
					<div class="form__icons">
						<img
							class="form__icon"
							src="data:image/svg+xml;base64,"
							alt="form_icon"
						/><img
							class="form__icon"
							src="data:image/svg+xml;base64,"
							alt="form_icon"
						/><img
							class="form__icon"
							src="data:image/svg+xml;base64,"
							alt="form_icon"
						/>
					</div>
					<%
						// Conseguimos el msg de error desde el request
						if (type != null && type.equals("login") && msg != null) { //Eliminamos el mensaje consumido
						%>
						<p><span class="form-info"> <%=msg%> </span></p>
						<%
						}
					%>
					<span class="form__span">o usar tu cuenta personal</span
					><input
						class="form__input"
						type="text"
						name="user"
						id="b-user"
						placeholder="User"
					/><input
						class="form__input"
						type="password"
						name="password"
						id="b-password"
						placeholder="Password"
					/><a class="form__link">Has olvidado la contraseña?</a
					><input type="submit" value="Iniciar Sesion" name="submit" class="form__button button submit">
				</form>
			</div>
			<div class="switch" id="switch-cnt">
				<div class="switch__circle"></div>
				<div class="switch__circle switch__circle--t"></div>
				<div class="switch__container" id="switch-c1">
					<h2 class="switch__title title">Bienvenido!</h2>
					<p class="switch__description description">
						Registra tus datos de usuario para poder experimentar las ventajas de nuestra plataforma. 
					</p>
					<button id="switch-to-reg" class="switch__button button switch-btn">
						Registro
					</button>
				</div>
				<div class="switch__container is-hidden" id="switch-c2">
					<h2 class="switch__title title">Hola de nuevo!</h2>
					<p class="switch__description description">
						Conectacte con tus credenciales para entrar en la zona de usuarios
					</p>
					<button class="switch__button button switch-btn">
						Iniciar Sesion
					</button>
				</div>
			</div>
		</div>
		<%
			// Conseguimos el msg de error desde el request
			if (type != null && type.equals("register")) { //Eliminamos el mensaje consumido
			%>
			<script type="text/javascript">
				setTimeout( () => {
					document.getElementById('switch-to-reg').click();
					console.log("Switch to register!");
				}, 20);
			</script>
			<%
			}
		%>
		<script src="js/login.js"></script>
		<script src="js/credenciales.js"></script>
	</body>
</html>
